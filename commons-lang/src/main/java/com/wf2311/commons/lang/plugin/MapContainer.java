package com.wf2311.commons.lang.plugin;
...
public class MapContainer extends LinkedHashMap<String, Object>{
  ...
  /**
   * 获取键名对应的布尔值；如果没有则返回默认值
   *
   * @param key      键名
   * @param defaults 预设默认值
   * @return
   */
  ...
  public char getAsChar(String key) {
    return getAsChar(key, '0');
  }

  /**
   * 获取键名对应的字符值；如果没有则返回默认值
   *
   * @param key      键名
   * @param defaults 预设默认值
   * @return
   */
  public char getAsChar(String key, char defaults) {
    Object value = get(key);
    if (value == null)
      return defaults;
  ...
  /**
   * 获取键名对应的整型值；如果没有则返回默认值
   *
   * @param key      键名
   * @param defaults 预设默认值
   * @return
   */
  public short getAsShort(String key, short defaults) {
    Object value = get(key);
    if (value == null)
      return defaults;
  ...
  /**
   * 获取键名对应的Date值；如果没有则返回默认值
   *
   * @param key      键名
   * @param defaults 预设默认值
   * @return
   */
  public Date getAsDate(String key, Date defaults) {
    Object value = get(key);
    if (value == null)
      return defaults;
    return DateUtils.parseToDate(value.toString());
  }
  ...
  public Date getAsDate(String key, String pattern, Date defaults) {
    ...
    if (value == null)
      return defaults;
    return DateUtils.parseToDate(value.toString(), pattern);
  }
  ...
  public Timestamp getAsTimestamp(String key) {
    return getAsTimestamp(key, null);
  }

  /**
   * 获取键名对应的Timestamp值；如果没有则返回默认值
   *
   * @param key      键名
   * @param defaults 预设默认值
   * @return
   */
  public Timestamp getAsTimestamp(String key, Timestamp defaults) {
    Object value = get(key);
    if (value == null)
      return defaults;
