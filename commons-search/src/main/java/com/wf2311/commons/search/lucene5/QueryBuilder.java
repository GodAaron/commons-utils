...
import com.wf2311.commons.search.lucene5.lang.LuceneUtil;
...
public class QueryBuilder {
    ...
    public QueryBuilder addFilter(String field, String value) {
        ...
        if (!ArrayUtils.isEmpty(tokens)) {
            for (String token : tokens) {
                filter.add(new Term(field, token));
            }
        } else {
            logger.warn("filter query field word can't analyzer -->" + value);
        }
        return this;
    }
    ...
    public QueryBuilder addMustNot(String field, String value) {
        String[] tokens = token(value);
        if (!ArrayUtils.isEmpty(tokens)) {
            for (String token : tokens) {
                mustNot.add(new Term(field, token));
            }
        } else {
            logger.warn("mustNot query field word can't analyzer -->" + value);
        }
        return this;
    }
    ...
    private List<String> reserveKeys = new ArrayList<String>() {{
        try {
    ...
    public String[] token(String str) {
        ...
        try {
            ...
            LuceneUtil.closeQuietly(stream);
        }
